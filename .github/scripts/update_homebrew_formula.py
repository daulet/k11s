...
def render_formula(
    *,
    ...
    return (
        ...
        "\n"
        "  def install\n"
        f'    libexec.install "{binary}", "{daemon_binary}"\n'
        f'    (bin/"{binary}").write_env_script libexec/"{binary}", K11SD_PATH: libexec/"{daemon_binary}"\n'
        "  end\n"
        "\n"
        "  test do\n"
        f'    assert_path_exists bin/"{binary}"\n'
        f'    assert_path_exists libexec/"{daemon_binary}"\n'
        "  end\n"
        "end\n"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render/update the Homebrew formula for k11s releases."
    )
    ...
    parser.add_argument("--binary", required=True, help="CLI binary name")
    parser.add_argument("--daemon-binary", required=True, help="Daemon binary name")
    ...
    parser.add_argument(
        "--desc",
        default="Speed-first CLI/TUI for Kubernetes navigation and operations",
